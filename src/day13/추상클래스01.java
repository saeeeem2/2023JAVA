...
public class 추상클래스01 {
	...
	public static void main(String[] args) {
		 ...
		 * - 부모클래스에서 상속을 받았을 때 특정메서드가 자식 클래스에서 자주 오버라이딩될 때 해당 메서드를 추상메서드로 작성
		 * - 상속을 받은 클래스에서 추상메서드가 있다면 반드시 구현해야 한다 => 구현하지 않으면 내 클래스도 추상 클래스가 됨*/
		
		Dog d=new Dog("뽀삐","개과");
		d.printInfo();
		d.howl();
		
		//Cat c=new Cat("나비","고양이과"); //추상클래스는 생성 불가
		d.printInfo();
		d.howl();
...
//Cat클래스 생성
//상속받은 추상메서드를 구현하지 않으려면 자신의 클래스도 추상클래스여야 한다
//추상클래스는 객체를 생성할 수 없다
